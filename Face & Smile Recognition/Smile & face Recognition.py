...
smile_cascade = cv2.CascadeClassifier('haarcascade_smile.xml') # We load the cascade for smiles.
cat_cascade = cv2.CascadeClassifier('haarcascade_catface.xml')
...
def detect(gray, frame): # We create a function that takes as input the image in black and white (gray) and the original image (frame), and that will return the same image with the detector rectangles. 
    ...
    for (x, y, w, h) in faces: # For each detected face:
        ...
        for (ex, ey, ew, eh) in eyes: # For each detected eye:
            cv2.rectangle(roi_color,(ex, ey),(ex+ew, ey+eh), (0, 0, 255), 2) # We paint a rectangle around the eyes, but inside the referential of the face.
        smiles = smile_cascade.detectMultiScale(roi_gray, 1.7, 22)
        for (sx, sy, sw, sh) in smiles:
            cv2.rectangle(roi_color, (sx, sy),(sx+sw, sy+sh), (0, 255,0), 2)
    cat_faces = cat_cascade.detectMultiScale(gray, 1.2, 15)
    for (cx, cy, cw, ch) in cat_faces:
        cv2.rectangle(frame, (cx, cy), (cx+cw, cy+ch), (255, 20, 147), 2)        
    return frame # We return the image with the detector rectangles.

video_capture = cv2.VideoCapture(0) # We turn the webcam on 0 is for built in camera 1 is for external.
